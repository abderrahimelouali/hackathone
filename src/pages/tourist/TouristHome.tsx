...
import { Search, Star, Clock, MapPin, Filter, ChefHat, Footprints, MountainSnow, Bike, Sparkles, ArrowLeft, ArrowRight, Heart, Waves, Car, Tent, Scissors, FlameKindling } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative h-[480px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center text-center p-4">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary-foreground/10 backdrop-blur-sm border border-primary-foreground/20 text-primary-foreground/90 text-sm mb-6">
              ...
              {lang === "fr" ? "Expériences Authentiques" : lang === "en" ? "Authentic Experiences" : "تجارب أصيلة"}
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="absolute bottom-0 inset-x-0">
          <div className="container mx-auto px-4">
            <div className="flex justify-center gap-8 md:gap-16 py-4 text-primary-foreground/80">
              ...
              <div className="text-center">
                ...
                <p className="text-xs opacity-70">{lang === "fr" ? "Visiteurs heureux" : lang === "en" ? "Happy visitors" : "زائر سعيد"}</p>
              </div>
            </div>
          </div>
        </motion.div>
      ...
      {/* Search & Filter */}
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass rounded-2xl shadow-card-hover p-5 -mt-8 relative z-10">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              <Search className="absolute start-4 top-3.5 w-5 h-5 text-muted-foreground" />
              <input type="text" placeholder={t("searchPlaceholder")} value={search} onChange={(e) => setSearch(e.target.value)} className="input-styled ps-12" />
            </div>
            <div className="flex items-center gap-3 bg-muted/50 rounded-xl px-4 py-2">
              ...
              <input type="range" min={50} max={1000} value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))} className="w-24 md:w-32 accent-primary" />
      ...
      <div className="container mx-auto px-4 py-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((activity, i) => (
            <motion.div key={activity.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05, duration: 0.4 }}>
              <Link to={`/activity/${activity.id}`} className="group card-interactive block overflow-hidden">
                <div className="relative h-56 overflow-hidden">
                  <img src={activity.images[0]} alt={activity.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" loading="lazy" />
                ...
                <div className="p-5">
                  <h3 className="font-bold text-foreground text-lg mb-2 group-hover:text-primary transition-colors line-clamp-1">{activity.title}</h3>
