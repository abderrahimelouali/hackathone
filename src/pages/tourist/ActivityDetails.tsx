...
import { Star, Clock, MapPin, Users, Calendar, ArrowRight, CheckCircle } from "lucide-react";
  ...
  if (!activity) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        <p className="text-muted-foreground text-lg">النشاط غير موجود</p>
        <button onClick={() => navigate("/")} className="mt-4 text-primary hover:underline">العودة للرئيسية</button>
      </div>
    );
  }

  const handleBook = () => {
    if (!date) { toast.error("يرجى اختيار التاريخ"); return; }
    addBooking({
      userId: user!.id,
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
              ...
              {activity.hasGuide && (
                <span className="bg-accent/10 text-accent px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1">
                  <CheckCircle className="w-4 h-4" /> مع مرشد سياحي
                </span>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: Star, label: "التقييم", value: `${activity.rating}/5`, color: "text-gold" },
              { icon: Clock, label: "المدة", value: activity.duration, color: "text-primary" },
              { icon: MapPin, label: "الموقع", value: activity.location, color: "text-accent" },
              { icon: Users, label: "المرشد", value: activity.hasGuide ? "متوفر" : "غير متوفر", color: "text-olive" },
            ].map((item) => (
        ...
        <div className="lg:col-span-1">
          <div className="bg-card rounded-xl shadow-warm p-6 sticky top-24 space-y-5">
            <div className="text-center">
              ...
              <span className="text-muted-foreground text-lg mr-2">د.م / شخص</span>
            </div>
            ...
            <div className="border-t border-border pt-4">
              <div className="flex justify-between text-sm text-muted-foreground mb-2">
                <span>{activity.price} د.م × {persons} أشخاص</span>
                <span>{activity.price * persons} د.م</span>
              </div>
              <div className="flex justify-between font-bold text-lg text-foreground">
                <span>المجموع</span>
                <span className="text-primary">{activity.price * persons} د.م</span>
              </div>
            </div>
