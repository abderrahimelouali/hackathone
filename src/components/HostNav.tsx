...
const HostNav = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-sidebar backdrop-blur-xl border-b border-sidebar-border">
          ...
          {links.map((l) => {
            ...
            return (
              <Link
                key={l.to}
                to={l.to}
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${active ? "bg-sidebar-primary text-sidebar-primary-foreground" : "text-sidebar-foreground/70 hover:bg-sidebar-accent"}`}
              >
      ...
      {open && (
        <div className="md:hidden border-t border-sidebar-border bg-sidebar p-4 space-y-1">
          {links.map((l) => (
            <Link key={l.to} to={l.to} onClick={() => setOpen(false)} className="flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-sidebar-accent text-sm font-medium text-sidebar-foreground">
              <l.icon className="w-4 h-4" />
              {l.label}
            </Link>
          ))}
